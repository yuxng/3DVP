...
for stage = 0:numel(opts.nWeak)-1
  ...
  if stage == 0 && isempty(opts.pPyramid.lambdas)
    fprintf('Computing lambdas... ');
    start = clock;
    ds = size(Is1);
    ds(1:end-1) = 1;
    Is1 = mat2cell2(Is1, ds);
...
if exist(crDir,'dir') && stage == 0 
  ...
  fs = bbGt('getFiles', {crDir});
  nImg = length(fs);
  assert(nImg > 0);
  if nImg > n
      fs = fs(:, randSample(nImg, n));
  end
  n = nImg;
  for i = 1:n
      fs{i} = [{opts.imreadf}, fs(i), opts.imreadp];
  end
  Is = cell(1,n);
  parfor i=1:n
      Is{i} = feval(fs{i}{:});
...
if positive && isstruct(opts.pJitter)
  ...
  Is = jitterImage(Is, opts.pJitter);
  ds = size(Is);
  ds(nd) = ds(nd)*ds(nd+1);
...
ds = size(Is);
cr = rem(ds(1:2), opts.pPyramid.pChns.shrink);
s = floor(cr/2) + 1;
e = ceil(cr/2);
Is = Is(s(1):end-e(1), s(2):end-e(2), :, :);
ds = size(Is);

if any(ds(1:2) < opts.modelDsPad)
    error('Windows too small.');
end
