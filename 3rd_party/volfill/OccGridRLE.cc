...
static inline void
write_fix_int(const void *p, size_t size, size_t nitems, FILE *fp)
{
...
static inline void
write_fix_float(const void *p, size_t size, size_t nitems, FILE *fp)
{
    swap_4(p, size * nitems);
    fwrite(p, size, nitems, fp);
    swap_4(p, size * nitems);
...
static inline void
write_fix_ushort(const void *p, size_t size, size_t nitems, FILE *fp)
{
...
static inline void
read_fix_int(void *p, size_t size, size_t nitems, FILE *fp)
{
...
static inline void
read_fix_float(void *p, size_t size, size_t nitems, FILE *fp)
{
    fread(p, size, nitems, fp);
    swap_4(p, size * nitems);
}
...
static inline void
read_fix_ushort(void *p, size_t size, size_t nitems, FILE *fp)
{
    fread(p, size, nitems, fp);
    swap_2(p, size * nitems);
}
...
OccGridRLE::write(char *filename)
{
    ...
    write_fix_int(&this->xdim, sizeof(int), 1, fp);
    write_fix_int(&this->ydim, sizeof(int), 1, fp);
    write_fix_int(&this->zdim, sizeof(int), 1, fp);
    write_fix_int(&this->axis, sizeof(int), 1, fp);
    write_fix_int(&this->flip, sizeof(int), 1, fp);
    write_fix_float(&this->origin, sizeof(float), 3, fp);
    write_fix_float(&this->resolution, sizeof(float), 1, fp);
    ...
    write_fix_int(&lengthByteCount, sizeof(int), 1, fp);
    write_fix_int(&elementByteCount, sizeof(int), 1, fp);
	...
	for (yy = 0; yy < this->ydim; yy++) {
		...
		write_fix_ushort(&length, sizeof(RunLength), 1, fp);
		...
		if (runType == OccGridRLE::CONSTANT_DATA) {
		    ...
		    write_fix_ushort(element, sizeof(OccElement), 1, fp);
		}
		else {
		    for (i=0; i<length; i++) {
			element = getNextElement();
...
OccGridRLE::read(char *filename)
{
    ...
    read_fix_int(&xd, sizeof(int), 1, fp);
    read_fix_int(&yd, sizeof(int), 1, fp);
    read_fix_int(&zd, sizeof(int), 1, fp);
    ...
    read_fix_int(&this->axis, sizeof(int), 1, fp);
    read_fix_int(&this->flip, sizeof(int), 1, fp);
    read_fix_float(&this->origin, sizeof(float), 3, fp);
    read_fix_float(&this->resolution, sizeof(float), 1, fp);

    int lengthByteCount, elementByteCount;
    read_fix_int(&lengthByteCount, sizeof(int), 1, fp);
    read_fix_int(&elementByteCount, sizeof(int), 1, fp);
	...
	for (yy = 0; yy < this->ydim; yy++) {
	    allocNewRun(yy, zz);
	    while (TRUE) {
		read_fix_ushort(&length, sizeof(RunLength), 1, fp);
		...
		if (runType == OccGridRLE::CONSTANT_DATA) {
		    read_fix_ushort(&element, sizeof(OccElement), 1, fp);
...
OccElement *
OccGridRLE::getScanline(int y, int z, OccElement *line)
{
    int i;
    assert (y<=ydim);
    assert (z<=zdim);
