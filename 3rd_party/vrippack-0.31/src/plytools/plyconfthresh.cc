...
usage(char *progname)
{
  ...
  fprintf (stderr, "       -c confidence_threshold (default = %g)\n", 
	   CONF_THRESH);
  fprintf (stderr, "\n");
  fprintf (stderr, "   This program will map all confidences above a\n");
  fprintf (stderr, "   cutoff threshold to 1.0 and those below to 0.0.\n");
  fprintf (stderr, "\n");
  exit(-1);
}
...
write_file()
{
  ...
  for (i = 0; i < nverts; i++) {
    ...
    if (vlist[i]->confidence > CONF_THRESH) {
       vlist[i]->confidence = 1.0;
    } else {
       vlist[i]->confidence = 0.0;
    }
  }
