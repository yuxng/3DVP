...
double   MIN_OVERLAP[3] = {0.7, 0.5, 0.5};                  // the minimum overlap required for evaluation
...
bool eval(string result_sha)
{
  ...
  // read image ids
  string ids_file = result_dir + "/" + "kitti_ids_val.txt";
  FILE *fp = fopen(ids_file.c_str(), "r");
  assert(fp != NULL);
  ...
  for (int32_t i = 0; i < N_TESTIMAGES; i++)
  {
    ...
    if (!gt_success)
    {
      printf("ERROR: Couldn't read: %s of ground truth!", file_name);
      return false;
    }
    if (!det_success)
    {
  ...
  if(eval_cyclist){
    ...
    if(   !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[0],aos[0],EASY)
      ...
      printf("Cyclist evaluation failed.");
      return false;
    }
    fclose(fp_det);
...
int32_t main (int32_t argc,char *argv[]) 
{
  // we need 2 arguments!
  if (argc != 3) {
    cout << "Usage: ./eval_detection result_sha overlap" << endl;
    return 1;
  }

  if (argc == 3) {
      MIN_OVERLAP[0] = atof(argv[2]);
  }
