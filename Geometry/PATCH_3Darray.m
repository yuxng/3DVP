...
%     valuesALL = valuesALL + 1; % Byung
  if crange(1) ~= crange(end)
    ...
    for loopJ = 1:cres
      ...
      hpat{loopJ} = patch(xco,yco,zco,cmap(loopJ,:), 'FaceAlpha', alpha);
    ...
    hpat = patch(xco,yco,zco,cmap(round(end/2),:),'FaceAlpha', alpha);
...
elseif colYN==0
  ...
  yco  = squeeze( facetsALL(:,2,:) )';
  zco  = squeeze( facetsALL(:,3,:) )';
  hpat = patch(xco,yco,zco,cmap,'FaceAlpha', alpha);
